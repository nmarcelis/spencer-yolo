  ...
  if (cam_image) {
    {
      ...
      camImageCopy_ = cam_image->image;     // Fix for OpenCV 2.4, otherwise results in segmentation fault...
...
void YoloObjectDetector::yolo() {
  ...
  if (!demoPrefix_ && viewImage_) {
    ...
    if (fullScreen_) {
      //cv::setWindowProperty("YOLO V3", cv::WND_PROP_FULLSCREEN, cv::WINDOW_FULLSCREEN);       // OpenCV >= 3.0
      cv::setWindowProperty("YOLO V3", CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);           // OpenCV 2.4 Fix
